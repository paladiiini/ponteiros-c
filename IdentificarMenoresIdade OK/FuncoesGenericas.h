...
void *encontrarMaior(void *dados[], int tam, FuncaoComparacao pfc) {
    ...
    for (int i = 0; i < tam; i++) {
        if (pfc(dados[i], dados[maior]) > 0) {
            maior = i;
        }
    }

    printf("\n**MAIOR VALOR DO VETOR**\n");
    printf("Valor de dados[%d]: %d\n", maior, *((int *)dados[maior]));
...
void FunPessoa(struct Pessoa *info1) {
    if (info1->idade < 18) {
        printf("%s é menor de idade com %d anos.\n", info1->nome, info1->idade);
    } else {
        printf("%s não é menor de idade com %d anos.\n", info1->nome, info1->idade);
    }
}
